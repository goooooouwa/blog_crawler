...
class Fetcher
  ...
  def self.start
    ...
    fetch_post(rss_items_file, ARGV[0], 0, ENV["DIRECTION"])
  end

  def self.fetch_post(rss_items_file, link, retry_count=0, direction="normal")
    ...
    if res.is_a?(Net::HTTPSuccess)
      ...
      if direction == 'reverse'
        fetch_post(rss_items_file, ENV["BASE_URL"] + rss_item[:next_link], 0, "reverse")
      else
        fetch_post(rss_items_file, ENV["BASE_URL"] + rss_item[:previous_link])
      end
    else
      @@logger.debug("request failed: #{res.inspect}")
      fetch_post(rss_items_file, link, retry_count + 1)
    end
  end
end
